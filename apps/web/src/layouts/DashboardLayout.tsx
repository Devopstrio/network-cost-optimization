...
const DashboardLayout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const navItems = [
    { name: 'Network Cost Hub', path: '/', icon: LayoutDashboard },
    { name: 'Traffic Flow Analytics', path: '/flows', icon: Activity },
    { name: 'Cost Breakdown', path: '/breakdown', icon: FileText },
    { name: 'Egress Optimization', path: '/egress', icon: TrendingDown },
    { name: 'VPC/VNet Peering', path: '/peering', icon: Share2 },
    { name: 'NAT Gateway Costs', path: '/nat', icon: Map },
    { name: 'CDN & Edge Services', path: '/cdn', icon: Globe },
    { name: 'Anomaly Detection', path: '/anomalies', icon: AlertTriangle },
    { name: 'Chargeback & Attribution', path: '/chargeback', icon: DollarSign },
    ...
    <div className="flex h-screen bg-slate-950 text-slate-200 overflow-hidden font-sans">
      ...
      <aside className={`bg-slate-900 border-r border-slate-800 transition-all duration-300 ${sidebarOpen ? 'w-64' : 'w-20'} flex flex-col`}>
        <div className="p-6 flex items-center gap-4 border-b border-slate-800">
          ...
          {sidebarOpen && <span className="font-bold text-xl tracking-tight bg-gradient-to-r from-white to-emerald-400 bg-clip-text text-transparent">Net.Fin</span>}
        </div>
          ...
          <div className="flex items-center gap-3 p-2 bg-slate-800/50 rounded-xl">
            ...
            {sidebarOpen && (
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold truncate">Cloud Economist</p>
                <p className="text-xs text-slate-500 truncate">Network Optimization</p>
              </div>
            )}
          </div>
        </div>
        ...
        <header className="h-16 bg-slate-900/50 backdrop-blur-md border-b border-slate-800 flex items-center justify-between px-6 z-10">
          <div className="flex items-center gap-4 flex-1">
            ...
            <div className="relative max-w-md w-full">
              ...
              <input 
                type="text" 
                placeholder="Search network services, NAT Gateways, Egress logs..." 
